...
public:
	...
	void parse();	// проводит синтаксический разбор 
...
private:
	...
	// Сравнение текущей лексемы с образцом. 
	// Текущая позиция в потоке лексем не изменяется.
	bool see(Token t)
	{
		return scanner_->token() == t; 
	}

	// Проверка совпадения текущей лексемы с образцом. 
	// Если лексема и образец совпадают, лексема изымается из потока.
	bool match(Token t)
	{
		if(scanner_->token() == t) {
			scanner_->nextToken();
	...
	void reportError(const string& message)
	{
        ...
        // Проверяем, совпадает ли данная лексема с образцом. 
	...
	// Эта функция пробегает по ассоциативному массиву variables_;
	// если находит нужную переменную, то возвращает ее номер; иначе
	...
	bool error_; // флаг ошибки; используется, чтобы определить, 
