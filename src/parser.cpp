...
void Parser::factor()
{
	...
	Множитель описывается следующими правилами:
           <factor> ::= <number> | <ident> | -<factor> | (<expression>) | 'READ'
	*/
	if(see(T_NUMBER)) {
		// Если встретили число, то преобразуем его в целое и 
		...
		// Если встретили переменную, то выгружаем значение, 
		...
		// Если встретили знак "-" и за ним <factor>, то 
		// инвертируем значение, лежащее на вершине стека.
		next();
		factor();
		codegen_->emit(INVERT);
	}
	else if(match(T_LPAREN)) {
		// Если встретили открывающую скобку, то следом может идти любое 
		// арифметическое выражение и обязательно закрывающая скобка.
		expression();
		mustBe(T_RPAREN);
	}
	else if(match(T_READ)) {
		// Если встретили зарезервированное слово READ, то записываем 
		// на вершину стека идет запись со стандартного ввода.
		codegen_->emit(INPUT);
	}
	else {
